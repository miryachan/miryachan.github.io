jQuery(document).ready(function($){
	...
	function hideWord($word) {
		...
		let i = 0;
		var designImages = document.getElementsByClassName('design-img');
		var codeImages = document.getElementsByClassName('code-img');
		var animationImages = document.getElementsByClassName('animation-img');

		
		function delayShowingImages(i, imageType, show){
			let images = "";

			console.log(imageType);

			
			if(imageType.compareTo("design")){
				images = designImages;
			}
			else if(imageType.equals("code")){
				images = codeImages;
			}
			else if(imageType.equals("animation")){
				images = animationImages;
			}
			//put these in each if statements if it works
			//im trying to see what image type it is (design code or animation) then doing the set timer thing!!!
			
			//setTimeout(() => {designImages[i].style.display = "block";}, 500); 

			alert(images);
		}
		
		setTimeout(function(){  
			//	if counter == 1 , its on the design word
			if(counter == 1){
				for(i = 0; i < designImages.length; i++){
					delayShowingImages(i, "design", true);
				}
			}
			else{
				for(i = 0; i < designImages.length; i++){
					delayShowingImages(i, "design", false);
				}
			}

			//	if counter == 2 , its on the code word
			if(counter == 2){
				for(i = 0; i < codeImages.length; i++){
					delayShowingImages(i, "code", true);
				}
			}
			else{
				for(i = 0; i < codeImages.length; i++){
					delayShowingImages(i, "code", false);
				}
			}

			//	if counter == 3 , its on the animation word
			if(counter == 3){
				for(i = 0; i < animationImages.length; i++){
					delayShowingImages(i, "animation", true);
				}
			}
			else{
				for(i = 0; i < animationImages.length; i++){
					delayShowingImages(i, "animation", false);
